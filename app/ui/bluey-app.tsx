...
export default function BlueyApp() {
  ...
  const [view, setView] = useState<"chat" | "reminders" | "games">("chat");
  ...
  return <main className="min-h-screen bg-[#fffaf0]">
    <header className="bg-[#175cd3] px-4 py-4 text-white shadow-md"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3"><div><h1 className="text-3xl font-black">Chat with Bluey</h1><p className="text-base text-blue-100">Your friendly everyday helper</p></div><div className="rounded-2xl bg-white/15 px-4 py-2 text-center"><strong className="text-xl">{subscribed ? "∞" : Math.max(0, 5-used)}</strong><span className="block text-sm">{subscribed ? "unlimited questions" : "free questions left"}</span></div></div><nav className="mx-auto mt-4 flex max-w-6xl gap-2" aria-label="Main navigation"><button onClick={() => setView("chat")} className={`focus-ring min-h-12 rounded-xl px-5 font-bold ${view === "chat" ? "bg-white text-blue-800" : "bg-blue-800 text-white"}`}>Chat</button><button onClick={() => setView("reminders")} className={`focus-ring min-h-12 rounded-xl px-5 font-bold ${view === "reminders" ? "bg-white text-blue-800" : "bg-blue-800 text-white"}`}>Reminders</button><button onClick={() => setView("games")} className={`focus-ring min-h-12 rounded-xl px-5 font-bold ${view === "games" ? "bg-white text-blue-800" : "bg-blue-800 text-white"}`}>Games</button></nav></header>
    {view === "reminders" ? <RemindersPanel sessionKey={sessionKey} /> : view === "games" ? <GamesPanel sessionKey={sessionKey} /> : <div className="mx-auto grid max-w-6xl gap-4 p-4 md:grid-cols-[320px_1fr]">
