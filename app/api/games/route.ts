...
export async function POST(request: Request) {
  try {
    const sessionKey = readSessionKey(request); const body = await request.json() as { gameType?: string; action?: string; score?: number; steps?: number };
    ...
    if (body.action === "start") {
      ...
      await db.from("audit_logs").insert({ event_type: "game_started", entity_type: "game", payload: { game_type: gameType, session_key: sessionKey.slice(0, 8) } });
      return NextResponse.json({ words });
    }
    await db.from("audit_logs").insert({ event_type: "game_completed", entity_type: "game", payload: { game_type: gameType, score: body.score ?? null, steps: body.steps ?? null, session_key: sessionKey.slice(0, 8) } });
