...
export async function GET(request: Request) {
  try {
    ...
    const [{ data: conversations, error: conversationError }, usageResult] = await Promise.all([
      db.from("conversations").select("id,title,created_at").or(`session_key.is.null,session_key.eq.${sessionKey}`).order("created_at", { ascending: true }),
    ...
    if (id) {
      const result = await db.from("messages").select("id,conversation_id,role,content,input_mode,image_url,created_at").eq("conversation_id", id).or(`session_key.is.null,session_key.eq.${sessionKey}`).order("created_at");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!conversationId) {
      const created = await db.from("conversations").insert({ title: content.slice(0, 42), session_key: sessionKey }).select("id").single();
      if (created.error) throw created.error;
      conversationId = created.data.id;
    }
    const savedUser = await db.from("messages").insert({ conversation_id: conversationId, role: "user", content, input_mode: inputMode, image_url: body.imageData ? "inline-image" : null, session_key: sessionKey }).select("id").single();
    ...
    const savedAssistant = await db.from("messages").insert({ conversation_id: conversationId, role: "assistant", content: reply.text, input_mode: "text", session_key: sessionKey }).select("id,conversation_id,role,content,input_mode,image_url,created_at").single();
